...
public class Spliter {
	...
	public File[] split(File srcFile, long maxFileSize) {
		...
		try {
			...
			int bufferSize = (int) (maxFileSize > MAX_FILE_SIZE ? MAX_FILE_SIZE  :  maxFileSize);
				...
				if (totalCount > maxFileSize) {
					...
					if (file.exists()) {
						if (this.overrideExistFile) {
							...
							if (b) {
								System.out.println("Over write exists file: "
										+ file.getAbsolutePath());
							} else
								System.out
										.println("Over write exists file failed: "
												+ file.getAbsolutePath());
						} else {
							System.out
									.println("File aread exists, you may move it to another place first: "
											+ file.getAbsolutePath());
							return null;
						}
					}
		...
		if (this.deleteSourceFile) {
			if (srcFile.delete())
				...
				System.out.println("Source file deleting fail: "
						+ srcFile.getAbsolutePath());
			}
		}
...
//			File file = new File(splitedFile[j].getParent()+ System.getProperty("file.separator") + newName[j]);
			...
			if (!renamed) {
				System.out.println("改名失败" + splitedFile[j].getAbsolutePath()
						+ " --> " + file.getAbsolutePath());
				return null;
			}
	...
	private static String[] getSplitedFileName(String orangeFileName,
			File[] splitedFile) {
